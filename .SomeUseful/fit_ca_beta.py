...
def extract_group_from_path(path: Path, valid_groups: set[int]) -> Optional[int]:
    """
    Conservative group-number parser.
    It avoids reading dates like 20260623 and avoids 'test1_20260623' as group 1.
    Best filenames are like: group_11.csv, g11.csv, CA_011.csv, blank_6.csv, sample_96.csv.
    """
    ...
    for pat in strong_patterns:
        for m in re.finditer(pat, stem):
            ...
            if num in valid_groups:
                # Avoid common false positive: test1_20260623
                before = stem[:m.start()]
                if before.endswith("test"):
                    continue
                return num

    # If the whole file stem is just a number.
...
def discover_ca_files(ca_dir: Path, valid_groups: Iterable[int], mapping_csv: Optional[Path] = None) -> tuple[dict[int, Path], list[Path]]:
    ...
    for p in sorted(ca_dir.rglob("*")):
        ...
        if g is None:
            unmapped.append(p)
            continue
        if g not in group_to_file:
            group_to_file[g] = p
        else:
            # Keep the first file but warn later by leaving duplicates visible in unmapped-like list.
            unmapped.append(p)
