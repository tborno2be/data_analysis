...
def fit_one(t, i, with_drift):
    """Fit one model with beta multi-start; returns (params dict, fitted curve or None)."""
    a0 = i[0] * t[0] ** 0.5
    ...
    for b0 in (0.3, 0.5, 0.8):
        try:
            ...
            continue
    out = {"A": np.nan, "beta": np.nan, "C": np.nan, "M": np.nan,
           "r2": np.nan, "beta_spread": np.nan}
    if not fits:
        return out, None
    ssr, popt, yfit = min(fits, key=lambda f: f[0])
    betas = [f[1][1] for f in fits]
    out.update(A=popt[0], beta=popt[1], C=popt[2], r2=r_squared(i, yfit),
