"""CA fitting layer: multi-start power-law+drift fit, adaptive tail cut, five gates, failure classing."""
...
import analysis_07082026.test_1.cv_run as cv_run
...
def gate_rows(rows):
    """Gates (ident, plateau, signal, A-consistency), analyte call, verdict; bin_excess is diagnostic only."""
...
def main(iso_dir, out_csv="ca_gates.csv"):
    """Fit every registered CA, gate, classify, write the table."""
    jobs = cv_run.ca_jobs(cv_run.read_sessions(iso_dir))
    if not jobs:
        print("no CA jobs found in session logs")
        return
    rows = []
    for ca_dir, sol, bg_ca, own_cv in jobs:
        potential, t, i = load_ca(Path(iso_dir) / ca_dir / "data.csv")
        f = fit_ca(t, i)
        row = {"file": ca_dir, "solution": sol, "bg_ca": bg_ca, "own_cv": own_cv,
               "potential": potential}
        if f is None:
            row.update({"beta": np.nan, "beta_rng": np.nan, "beta_spread_max": np.nan,
                        "A": np.nan, "C": np.nan, "M": np.nan, "r2": np.nan,
                        "sigma_noise": np.nan, "t_max_used": np.nan, "snr": np.nan,
                        "bin_excess": np.nan, "mad_ratio": np.nan})
        else:
            f.pop("resid", None)
            row.update(f)
        rows.append(row)
    ...
    with open(out_csv, "w", newline="", encoding="utf-8") as fh:
        ...
        w.writeheader()
        w.writerows(rows)
    print(f"{len(rows)} CAs -> {out_csv}")
    return rows


def session_betas(iso_dir):
    """{own_cv_dir: certified beta} for feeding the CV order input; gated CAs only."""
    rows = main(iso_dir, out_csv="ca_gates.csv") or []
    return {r["own_cv"]: r["beta"] for r in rows if r.get("pass_all") and r["own_cv"]}
