"""Single-round polished-electrode session: blank then sample, one CV + one CA each, with polish logging."""
...
class SessionLog:
    """Append-per-step timeline CSV; never overwrites."""
    ...
    def record(self, solution, action, t_start, t_end=None, out_dir=None):
...
def run_session(port: str) -> None:
    """Single round: record polish, blank block, rinse, sample block."""
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log = SessionLog(ISO_DIR / f"session_polish_{stamp}.csv")
    LOG.info("polished-electrode session started; log -> %s", log.path)
    log.record("none", "run_start", datetime.now())

    t0 = datetime.now()
    _confirm("Confirm the electrode has been POLISHED")
    log.record("none", "polish", t0)
    ...
    LOG.info("session finished at %s; timeline -> %s",
