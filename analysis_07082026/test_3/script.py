"""Multi-scan-rate session: for each solution, sweep CV scan rates (each with a CA) to test diffusion control."""
...
SCAN_RATES_MV_S = (20, 50, 100, 200, 500)

CV_BASE = {"e_begin_v": 0, "e_vertex1_v": 0, "e_vertex2_v": 0.6,
           "n_scans": 5, "t_equilibrium_s": 5, "e_step_mv": 5}
...
def run_CV(port: str, scan_rate_mv_s: int) -> Path:
    """One CV at the given scan rate into the next Cv<n> folder; returns the folder."""
    out_dir = ISO_DIR / f"Cv{_next_index(ISO_DIR, 'Cv')}"
    params = {**CV_BASE, "scan_rate_mv_s": scan_rate_mv_s}
    em, lines = _measure(port, EmStat4X.genmscript_CV(**params), out_dir)
    em.saveraw_CV(lines, out_dir / "raw.txt")
    dt = params["e_step_mv"] / scan_rate_mv_s if WITH_TIME else None
    em.savecsv_CV(lines, out_dir / "data.csv", dt=dt)
    LOG.info("CV %d mV/s saved to %s", scan_rate_mv_s, out_dir)
    return out_dir
...
class SessionLog:
    ...
    FIELDS = ["step", "solution", "action", "scan_rate_mv_s", "t_start", "t_end", "elapsed_s", "out_dir"]
    ...
    def record(self, solution, action, t_start, t_end=None, out_dir=None, scan_rate=""):
        ...
        LOG.info("logged: %s %s %s (%.0f s)", solution, action, scan_rate, row["elapsed_s"])


def _rate_cv(port, log, solution, rate):
    """settle -> CV at one rate, timed and logged."""
    t0 = datetime.now()
    _wait(WAIT_BEFORE_CV_S, f"{solution} {rate}mV/s: settle before CV")
    log.record(solution, f"wait_{WAIT_BEFORE_CV_S}s", t0, scan_rate=rate)
    t0 = datetime.now()
    out = run_CV(port, rate)
    log.record(solution, "CV", t0, out_dir=out, scan_rate=rate)


def _solution_block(port, log, solution):
    """One load, sweep all CV scan rates, then a single closing CA."""
    t0 = datetime.now()
    _confirm(f"Load {solution.upper()} onto the electrode")
    log.record(solution, "load", t0)
    for rate in SCAN_RATES_MV_S:
        _rate_cv(port, log, solution, rate)
    t0 = datetime.now()
    _wait(WAIT_BETWEEN_S, f"{solution}: relax before CA")
    log.record(solution, f"wait_{WAIT_BETWEEN_S}s", t0)
    t0 = datetime.now()
    out = run_CA(port)
...
def run_session(port: str) -> None:
    """Rounds of blank -> rinse -> sample (each a full scan-rate sweep) until terminated."""
    ...
    LOG.info("scan rates: %s mV/s", SCAN_RATES_MV_S)

    while True:
        # _solution_block(port, log, "blank")

        # t0 = datetime.now()
        # _confirm("Rinse the electrode with pure water")
        # log.record("none", "rinse", t0)

        _solution_block(port, log, "sample")
    ...
    log.record("none", "rinse", t0)
    _solution_block(port, log, "blank_final")
